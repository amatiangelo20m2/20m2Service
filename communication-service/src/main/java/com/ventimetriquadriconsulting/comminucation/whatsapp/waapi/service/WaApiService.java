...
public class WaApiService extends WaApiInterface {
    ...
    @Override
    public ListInstanceResponse restrieveInstances() {
        log.info("Retrieve list instances info for instance..");
        ...
        return waapiWebClient
                ...
                .map(responseBody -> {
                    log.info("Response from [api/v1/instances] - {}", responseBody);
                    haveSomeTimeToSleep(500);
                    try {
                        ObjectMapper objectMapper = new ObjectMapper();
                        return objectMapper.readValue(responseBody, ListInstanceResponse.class);
    ...
    @Override
    public ClientStatusResponse retrieveInstanceStatus(String instanceId) {
        log.info("Retrieve status wa api instance with id {}", instanceId);
        return waapiWebClient
                .get()
                .uri("api/v1/instances/" + instanceId +"/client/status")
                .retrieve()
                .onStatus(
                        HttpStatusCode::is4xxClientError,
                        clientResponse -> Mono.error(new Exception("Client Error"))
                ...
                .bodyToMono(String.class)
                .map(responseBody -> {
                    log.info("Response from [api/v1/instances/{}/client/status] - {}",instanceId, responseBody);
                    haveSomeTimeToSleep(500);
                    try {
                        ObjectMapper objectMapper = new ObjectMapper();
                        return objectMapper.readValue(responseBody, ClientStatusResponse.class);
                    } catch (JsonProcessingException e) {
                        throw new RuntimeException(e);
                    }
                })
